...
export default function Home() {
  // Initialize state for tasks and filter using useState
  ...
  const handleAddTask = () => {  
    const newTask = {
      id: tasks.length + 1,  // Simple ID generation strategy
      text: "text",
      completed: false
  ...
  const handleToggleTask = (taskId) => {
    ...
    setTasks(updatedTasks);  // Update the tasks array with the toggled task
  };

  // удалить таск
  const handleDeleteTask = (taskId) => {
    const remainingTasks = tasks.filter(task => task.id !== taskId);
    setTasks(remainingTasks);  // Set tasks to the remaining tasks after deletion
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="bg-gray-800 rounded p-4">
        ...
        <ul>
          ...
          {tasks.map(task => (<li key={task.id} className="flex justify-between items-center p-2 bg-gray-900 rounded mb-2">
